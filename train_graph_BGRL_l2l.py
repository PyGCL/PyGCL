...
from GCL.eval import LR_classification, SVM_classification
from GCL.utils import seed_everything
from torch_geometric.nn import GCNConv, GINConv

from utils import load_node_dataset, get_activation, load_graph_dataset
...
def bgrl_loss(x: torch.FloatTensor, y: torch.FloatTensor):
...
def train(model, optimizer, loader, device, args):
    ...
    for data in loader:
        ...
        loss = bgrl_loss(h2_target.detach(), h1_pred)
        loss += bgrl_loss(h1_target.detach(), h2_pred)
        loss = loss.mean()
        loss.backward()
        optimizer.step()
...
def test(model, loader, device, seed):
    ...
    for data in loader:
        ...
        g1, g2 = global_add_pool(h1, data.batch), global_add_pool(h2, data.batch)
...
def main():
    ...
    parser.add_argument('--param_path', type=str, default='params/GRACE/ptc_mr.json')
