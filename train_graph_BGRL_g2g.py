...
from models.BGRL import BGRL_G2G as BGRL
...
def train(model, optimizer, loader, device, args):
    ...
    for data in loader:
        ...
        optimizer.zero_grad()
        _, _, g1_pred, g2_pred, g1_target, g2_target = model(data.x, data.batch, data.edge_index)

        loss = bgrl_loss(g1_pred, g2_target.detach())
        loss += bgrl_loss(g2_pred, g1_target.detach())
        loss = loss.mean()
        loss.backward()
        optimizer.step()
